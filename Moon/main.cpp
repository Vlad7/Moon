...
Moon moon1(1.0f, 36, 18, false, 2);    // radius, sectors, stacks, non-smooth (flat) shading, Y-up
Moon moon2(1.0f, 36, 18, true,  2);    // radius, sectors, stacks, smooth(default), Y-up
...
void showInfo()
{
    ...
    ss << "Moon Radius: " << moon2.getRadius() << std::ends;
    drawString(ss.str().c_str(), 1, screenHeight-TEXT_HEIGHT, color, font);
    ss.str("");

    ss << "Sector Count: " << moon2.getSectorCount() << std::ends;
    drawString(ss.str().c_str(), 1, screenHeight-(2*TEXT_HEIGHT), color, font);
    ss.str("");

    ss << "Stack Count: " << moon2.getStackCount() << std::ends;
    drawString(ss.str().c_str(), 1, screenHeight-(3*TEXT_HEIGHT), color, font);
    ss.str("");

    ss << "Vertex Count: " << moon2.getVertexCount() << std::ends;
    drawString(ss.str().c_str(), 1, screenHeight-(4*TEXT_HEIGHT), color, font);
    ss.str("");

    ss << "Index Count: " << moon2.getIndexCount() << std::ends;
...
void keyboardCB(unsigned char key, int x, int y)
{
    ...
    case ' ':
        moon1.reverseNormals();
        moon2.reverseNormals();
        break;
